...
GuiInstallStart::GuiInstallStart(Window* window) : GuiComponent(window),
  mMenu(window, _("INSTALL GAMESTATION").c_str())
{
	addChild(&mMenu);

	// available install storage
	std::vector<std::string> availableStorage = GamestationSystem::getInstance()->getAvailableInstallDevices();
	...
	std::vector<std::string> availableArchitecture = GamestationSystem::getInstance()->getAvailableInstallArchitectures();
