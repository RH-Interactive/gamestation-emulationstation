...
void GuiMenu::createInputTextRow(GuiSettings *gui, std::string title, const char *settingsID, bool password) {
    ...
    ed = std::make_shared<TextComponent>(window, ((password &&
                                                   GamestationConf::getInstance()->get(settingsID) != "")
                                                  ? "*********" : GamestationConf::getInstance()->get(
    ...
    auto updateVal = [ed, settingsID, password](const std::string &newVal) {
        ...
        GamestationConf::getInstance()->set(settingsID, newVal);
    }; // ok callback (apply new value to ed)

    row.makeAcceptInputHandler([this, title, updateVal, settingsID] {
        if (Settings::getInstance()->getBool("UseOSK")) {
            mWindow->pushGui(
                new GuiTextEditPopupKeyboard(mWindow, title, GamestationConf::getInstance()->get(settingsID),
                                        updateVal, false));
        } else {
            mWindow->pushGui(
                new GuiTextEditPopup(mWindow, title, GamestationConf::getInstance()->get(settingsID),
                                       updateVal, false));
        }
    });
    gui->addRow(row);
...
GuiMenu::GuiMenu(Window *window) : GuiComponent(window), mMenu(window, _("MAIN MENU").c_str()), mVersion(window) {
...
#if ENABLE_KODI == 1
    if (GamestationConf::getInstance()->get("kodi.enabled") == "1") {
      addEntry(_("KODI MEDIA CENTER").c_str(), 0x777777FF, true,
                 [this] {
                     Window *window = mWindow;

                     if (!GamestationSystem::getInstance()->launchKodi(window)) {
    ...
    if (GamestationConf::getInstance()->get("system.es.menu") != "bartop") {
      addEntry(_("SYSTEM SETTINGS").c_str(), 0x777777FF, true,
			   ...
			   auto version = std::make_shared<TextComponent>(mWindow,
			   						  GamestationSystem::getInstance()->getVersion(),
			   						  Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
			   informationsGui->addWithLabel(_("VERSION"), version);
			   bool warning = GamestationSystem::getInstance()->isFreeSpaceLimit();
			   auto space = std::make_shared<TextComponent>(mWindow,
			   						GamestationSystem::getInstance()->getFreeSpaceInfo(),
			   						Font::get(FONT_SIZE_MEDIUM),
			   						warning ? 0xFF0000FF : 0x777777FF);
			   informationsGui->addWithLabel(_("DISK USAGE"), space);

			   // various informations
			   std::vector<std::string> infos = GamestationSystem::getInstance()->getSystemInformations();
			   ...
			   // support
			   {
				 ...
				 mWindow->pushGui(new GuiMsgBox(mWindow, _("CREATE A SUPPORT FILE ?"), _("YES"),
								[this] {
								  if(GamestationSystem::getInstance()->generateSupportFile()) {
			   ...
			   mWindow->pushGui(informationsGui);
                     ...
                     std::vector<std::string> availableStorage = GamestationSystem::getInstance()->getAvailableStorageDevices();
                     std::string selectedStorage = GamestationSystem::getInstance()->getCurrentStorage();
				   ...
				   optionsStorage->add(vname, (*it), selectedStorage == std::string("DEV "+tokens.at(1)));
                     ...
                     auto language_choice = std::make_shared<OptionListComponent<std::string> >(window, _("LANGUAGE"),
                                                                                                false);
                     std::string language = GamestationConf::getInstance()->get("system.language");
		     ...
		     if(currentOverclock == "") currentOverclock = "none";
                     std::vector<std::string> availableOverclocking = GamestationSystem::getInstance()->getAvailableOverclocking();
		     ...
		     if(isOneSet == false) {
			   ...
			   GuiSettings *updateGui = new GuiSettings(mWindow, _("UPDATES").c_str());
                             ...
                             updates_enabled->setState(
                                     GamestationConf::getInstance()->get("updates.enabled") == "1");
			   updateGui->addWithLabel(_("AUTO UPDATES"), updates_enabled);
                             ...
                             updateGui->addSaveFunc([updates_enabled] {
                                 GamestationConf::getInstance()->set("updates.enabled",
                                                                  updates_enabled->getState() ? "1" : "0");
                                 GamestationConf::getInstance()->saveGamestationConf();
                             });
		     ...
		     // backup
		     {
			   ...
			   GuiSettings *kodiGui = new GuiSettings(mWindow, _("KODI SETTINGS").c_str());
                             ...
                             kodiEnabled->setState(GamestationConf::getInstance()->get("kodi.enabled") == "1");
			   kodiGui->addWithLabel(_("ENABLE KODI"), kodiEnabled);
                             ...
                             kodiAtStart->setState(
                                     GamestationConf::getInstance()->get("kodi.atstartup") == "1");
			   kodiGui->addWithLabel(_("KODI AT START"), kodiAtStart);
                             auto kodiX = std::make_shared<SwitchComponent>(mWindow);
                             kodiX->setState(GamestationConf::getInstance()->get("kodi.xbutton") == "1");
			   kodiGui->addWithLabel(_("START KODI WITH X"), kodiX);
                             kodiGui->addSaveFunc([kodiEnabled, kodiAtStart, kodiX] {
                                 GamestationConf::getInstance()->set("kodi.enabled",
                                                                  kodiEnabled->getState() ? "1" : "0");
                                 GamestationConf::getInstance()->set("kodi.atstartup",
                                                                  kodiAtStart->getState() ? "1" : "0");
                                 GamestationConf::getInstance()->set("kodi.xbutton",
                                                                  kodiX->getState() ? "1" : "0");
                                 GamestationConf::getInstance()->saveGamestationConf();
                             });
		     ...
		     // install
		     {
			   ...
			   securityEnabled->setState(GamestationConf::getInstance()->get("system.security.enabled") == "1");
			   securityGui->addWithLabel(_("ENFORCE SECURITY"), securityEnabled);

			   auto rootpassword = std::make_shared<TextComponent>(mWindow,
									       GamestationSystem::getInstance()->getRootPassword(),
									       Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
			   ...
			   securityGui->addSaveFunc([this, securityEnabled] {
				 ...
				 GamestationConf::getInstance()->set("system.security.enabled",
			   ...
			   mWindow->pushGui(securityGui);
                     ...
                     s->addSaveFunc([overclock_choice, window, language_choice, language, optionsStorage, selectedStorage] {
                         ...
                         if (optionsStorage->changed()) {
                             GamestationSystem::getInstance()->setStorage(optionsStorage->getSelected());
                             reboot = true;
                         }
			     ...
			     (Settings::getInstance()->getString("Overclock") == "" && overclock_choice->getSelected() != "none")) {
                             ...
                             GamestationSystem::getInstance()->setOverclock(overclock_choice->getSelected());
                             reboot = true;
                         }
                         if (language != language_choice->getSelected()) {
                             GamestationConf::getInstance()->set("system.language",
    ...
    addEntry(_("GAMES SETTINGS").c_str(), 0x777777FF, true,
             [this] {
                 ...
                 if (GamestationConf::getInstance()->get("system.es.menu") != "bartop") {
                     ...
                     s->addSaveFunc([ratio_choice] {
                         GamestationConf::getInstance()->set("global.ratio", ratio_choice->getSelected());
                         GamestationConf::getInstance()->saveGamestationConf();
                     });
                 }
                 // smoothing
                 auto smoothing_enabled = std::make_shared<OptionListComponent<std::string>>(mWindow, _("SMOOTH GAMES"));
		 smoothing_enabled->add(_("AUTO"), "auto", GamestationConf::getInstance()->get("global.smooth") != "0" && GamestationConf::getInstance()->get("global.smooth") != "1");
		 smoothing_enabled->add(_("ON"),   "1",    GamestationConf::getInstance()->get("global.smooth") == "1");
		 smoothing_enabled->add(_("OFF"),  "0",    GamestationConf::getInstance()->get("global.smooth") == "0");
		 ...
		 rewind_enabled->add(_("AUTO"), "auto", GamestationConf::getInstance()->get("global.rewind") != "0" && GamestationConf::getInstance()->get("global.rewind") != "1");
		 rewind_enabled->add(_("ON"),   "1",    GamestationConf::getInstance()->get("global.rewind") == "1");
		 rewind_enabled->add(_("OFF"),  "0",    GamestationConf::getInstance()->get("global.rewind") == "0");
		 ...
		 autosave_enabled->add(_("AUTO"), "auto", GamestationConf::getInstance()->get("global.autosave") != "0" && GamestationConf::getInstance()->get("global.autosave") != "1");
		 autosave_enabled->add(_("ON"),   "1",    GamestationConf::getInstance()->get("global.autosave") == "1");
		 autosave_enabled->add(_("OFF"),  "0",    GamestationConf::getInstance()->get("global.autosave") == "0");
                 ...
                 auto shaders_choices = std::make_shared<OptionListComponent<std::string> >(mWindow, _("SHADERS SET"),
                                                                                            false);
                 std::string currentShader = GamestationConf::getInstance()->get("global.shaderset");
		 ...
		 integerscale_enabled->add(_("AUTO"), "auto", GamestationConf::getInstance()->get("global.integerscale") != "0" && GamestationConf::getInstance()->get("global.integerscale") != "1");
		 integerscale_enabled->add(_("ON"),   "1",    GamestationConf::getInstance()->get("global.integerscale") == "1");
		 integerscale_enabled->add(_("OFF"),  "0",    GamestationConf::getInstance()->get("global.integerscale") == "0");
                 s->addWithLabel(_("INTEGER SCALE (PIXEL PERFECT)"), integerscale_enabled);
                 s->addSaveFunc([integerscale_enabled] {
                     GamestationConf::getInstance()->set("global.integerscale", integerscale_enabled->getSelected());
                     GamestationConf::getInstance()->saveGamestationConf();
                 });
		 ...
		 // decorations
		 {
		   ...
		   for (auto it = decorations_item.begin(); it != decorations_item.end(); it++) {
		     decorations->add(*it, *it,
				      (GamestationConf::getInstance()->get("global.bezel") == *it)
				      ||
				      (GamestationConf::getInstance()->get("global.bezel") == "none" && *it == _("NONE"))
				      ||
				      (GamestationConf::getInstance()->get("global.bezel") == "" && *it == _("AUTO"))
				      );
		   }
		   s->addWithLabel(_("DECORATION"), decorations);
		   s->addSaveFunc([decorations] {
		       GamestationConf::getInstance()->set("global.bezel", decorations->getSelected() == _("NONE") ? "none" : decorations->getSelected() == _("AUTO") ? "" : decorations->getSelected());
		       GamestationConf::getInstance()->saveGamestationConf();
		     });
		 }
                     ...
                     // Retroachievements
                     {
                         ...
                         std::function<void()> openGui = [this] {
                             ...
                             retroachievements_enabled->setState(
                                     GamestationConf::getInstance()->get("global.retroachievements") == "1");
                             ...
                             retroachievements_hardcore_enabled->setState(
                                     GamestationConf::getInstance()->get("global.retroachievements.hardcore") == "1");
                             ...
                             retroachievements_leaderboards_enabled->setState(
                                     GamestationConf::getInstance()->get("global.retroachievements.leaderboards") == "1");
                             ...
                             retroachievements_verbose_enabled->setState(
                                     GamestationConf::getInstance()->get("global.retroachievements.verbose") == "1");
                             ...
                             retroachievements_screenshot_enabled->setState(
                                     GamestationConf::getInstance()->get("global.retroachievements.screenshot") == "1");
                             ...
                             retroachievements->addSaveFunc([retroachievements_enabled, retroachievements_hardcore_enabled, retroachievements_leaderboards_enabled,
                                 ...
                                 GamestationConf::getInstance()->set("global.retroachievements",
                                                                  retroachievements_enabled->getState() ? "1" : "0");
                                 GamestationConf::getInstance()->set("global.retroachievements.hardcore",
                                                                  retroachievements_hardcore_enabled->getState() ? "1" : "0");
                                 GamestationConf::getInstance()->set("global.retroachievements.leaderboards",
                                                                  retroachievements_leaderboards_enabled->getState() ? "1" : "0");
                                 GamestationConf::getInstance()->set("global.retroachievements.verbose",
                                                                  retroachievements_verbose_enabled->getState() ? "1" : "0");
                                 GamestationConf::getInstance()->set("global.retroachievements.screenshot",
			 ...
			 std::vector<BiosSystem> biosInformations = GamestationSystem::getInstance()->getBiosInformations();
			 ...
			 mWindow->pushGui(configuration);
		       ...
		       s->addRow(row);
                 ...
                 s->addSaveFunc([smoothing_enabled, rewind_enabled, shaders_choices, autosave_enabled] {
                     GamestationConf::getInstance()->set("global.smooth", smoothing_enabled->getSelected());
                     GamestationConf::getInstance()->set("global.rewind", rewind_enabled->getSelected());
                     GamestationConf::getInstance()->set("global.shaderset", shaders_choices->getSelected());
                     GamestationConf::getInstance()->set("global.autosave", autosave_enabled->getSelected());
                     GamestationConf::getInstance()->saveGamestationConf();
                 });
                 mWindow->pushGui(s);
             }
    ...
    if (GamestationConf::getInstance()->get("system.es.menu") != "bartop") {
      ...
      addEntry(_("UI SETTINGS").c_str(), 0x777777FF, true,
		 ...
		 auto s = new GuiSettings(mWindow, _("UI SETTINGS").c_str());
		     ...
		     std::string currentDevice = GamestationConf::getInstance()->get("global.videooutput");
		     if (currentDevice.empty()) currentDevice = "auto";

		     std::vector<std::string> availableVideo = GamestationSystem::getInstance()->getAvailableVideoOutputDevices();
			 ...
			 if (currentDevice != optionsVideo->getSelected()) {
			   GamestationConf::getInstance()->set("global.videooutput", optionsVideo->getSelected());
			   GamestationConf::getInstance()->saveGamestationConf();
			   this->mWindow->pushGui(
                     ...
                     s->addSaveFunc([overscan_enabled] {
                         if (Settings::getInstance()->getBool("Overscan") != overscan_enabled->getState()) {
                             ...
                             GamestationSystem::getInstance()->setOverscan(overscan_enabled->getState());
                         }
                     });
                     // screensaver time
    ...
    addEntry(_("SOUND SETTINGS").c_str(), 0x777777FF, true,
	       ...
	       auto s = new GuiSettings(mWindow, _("SOUND SETTINGS").c_str());
                 ...
                 sounds_enabled->setState(!(GamestationConf::getInstance()->get("audio.bgmusic") == "0"));
		 ...
		 auto optionsAudio = std::make_shared<OptionListComponent<std::string> >(mWindow, _("OUTPUT DEVICE"),
											 false);
                 std::string currentDevice = GamestationConf::getInstance()->get("audio.device");
                 if (currentDevice.empty()) currentDevice = "auto";

		 std::vector<std::string> availableAudio = GamestationSystem::getInstance()->getAvailableAudioOutputDevices();
		 std::string selectedAudio = GamestationSystem::getInstance()->getCurrentAudioOutputDevice();
		 ...
		 if (GamestationConf::getInstance()->get("system.es.menu") != "bartop") {
		   ...
		   s->addWithLabel(_("OUTPUT DEVICE"), optionsAudio);
		     ...
		     bool v_need_reboot = false;
                     ...
                     GamestationConf::getInstance()->set("audio.volume",
                                                      std::to_string((int) round(volume->getValue())));

                     GamestationConf::getInstance()->set("audio.bgmusic",
                     ...
                     if (currentDevice != optionsAudio->getSelected()) {
                         GamestationConf::getInstance()->set("audio.device", optionsAudio->getSelected());
                         GamestationSystem::getInstance()->setAudioOutputDevice(optionsAudio->getSelected());
			 v_need_reboot = true;
                     }
                     GamestationConf::getInstance()->saveGamestationConf();
		     if(v_need_reboot) {
      ...
      addEntry(_("NETWORK SETTINGS").c_str(), 0x777777FF, true,
                 [this] {
                     ...
                     auto status = std::make_shared<TextComponent>(mWindow,
                                                                   GamestationSystem::getInstance()->ping() ? _("CONNECTED")
								   : _("NOT CONNECTED"),
                                                                   Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
                     s->addWithLabel(_("STATUS"), status);
                     auto ip = std::make_shared<TextComponent>(mWindow, GamestationSystem::getInstance()->getIpAdress(),
                                                               Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
                     ...
                     bool baseEnabled = GamestationConf::getInstance()->get("wifi.enabled") == "1";
                     enable_wifi->setState(baseEnabled);
                     s->addWithLabel(_("ENABLE WIFI"), enable_wifi);

                     // window, title, settingstring,
                     const std::string baseSSID = GamestationConf::getInstance()->get("wifi.ssid");
                     createInputTextRow(s, _("WIFI SSID"), "wifi.ssid", false);
                     const std::string baseKEY = GamestationConf::getInstance()->get("wifi.key");
                     ...
                     s->addSaveFunc([baseEnabled, baseSSID, baseKEY, enable_wifi, window] {
                         ...
                         GamestationConf::getInstance()->set("wifi.enabled", wifienabled ? "1" : "0");
                         std::string newSSID = GamestationConf::getInstance()->get("wifi.ssid");
                         std::string newKey = GamestationConf::getInstance()->get("wifi.key");
                         GamestationConf::getInstance()->saveGamestationConf();
                         if (wifienabled) {
                             if (baseSSID != newSSID
                                 || baseKEY != newKey
                                 || !baseEnabled) {
                                 if (GamestationSystem::getInstance()->enableWifi(newSSID, newKey)) {
                                     window->pushGui(
                             ...
                             GamestationSystem::getInstance()->disableWifi();
                         }
                     });
                     mWindow->pushGui(s);
	       ...
	       auto s = new GuiSettings(mWindow, _("QUIT").c_str());
                 ...
                 row.makeAcceptInputHandler([window] {
                     window->pushGui(new GuiMsgBox(window, _("REALLY RESTART?"), _("YES"),
                                                   [] {
                                                       if (GamestationSystem::getInstance()->reboot() != 0) {
                                                           LOG(LogWarning) <<
                 ...
                 row.makeAcceptInputHandler([window] {
                     window->pushGui(new GuiMsgBox(window, _("REALLY SHUTDOWN?"), _("YES"),
                                                   [] {
                                                       if (GamestationSystem::getInstance()->shutdown() != 0) {
                                                           LOG(LogWarning) <<
                 ...
                 row.makeAcceptInputHandler([window] {
                     window->pushGui(new GuiMsgBox(window, _("REALLY SHUTDOWN WITHOUT SAVING METADATAS?"), _("YES"),
                                                   [] {
                                                       if (GamestationSystem::getInstance()->fastShutdown() != 0) {
                                                           LOG(LogWarning) <<
                                                                           "Shutdown terminated with non-zero result!";
                                                       }
                                                   }, _("NO"), nullptr));
    ...
    mVersion.setText("RH GAMESTATION" + strToUpper(PROGRAM_VERSION_STRING));
...
void GuiMenu::popSystemConfigurationGui(SystemData *systemData, std::string previouslySelectedEmulator) const {
    ...
    for (auto it = systemData->getEmulators()->begin(); it != systemData->getEmulators()->end(); it++) {
        ...
        if (previouslySelectedEmulator != "") {
            ...
            found = (GamestationConf::getInstance()->get(systemData->getName() + ".emulator") == curEmulatorName);
        }
        if (found) {
    ...
    for (auto emulator = systemData->getEmulators()->begin();
        ...
        if (selectedEmulator == emulator->first) {
            for (auto core = emulator->second->begin(); core != emulator->second->end(); core++) {
                bool found = (GamestationConf::getInstance()->get(systemData->getName() + ".core") == *core);
                selected = selected || found;
    ...
    // smoothing
    auto smoothing_enabled = std::make_shared<OptionListComponent<std::string>>(mWindow, _("SMOOTH GAMES"));
    smoothing_enabled->add(_("AUTO"), "auto", GamestationConf::getInstance()->get(systemData->getName() + ".smooth") != "0" && GamestationConf::getInstance()->get(systemData->getName() + ".smooth") != "1");
    smoothing_enabled->add(_("ON"),   "1",    GamestationConf::getInstance()->get(systemData->getName() + ".smooth") == "1");
    smoothing_enabled->add(_("OFF"),  "0",    GamestationConf::getInstance()->get(systemData->getName() + ".smooth") == "0");
    systemConfiguration->addWithLabel(_("SMOOTH GAMES"), smoothing_enabled);
    // rewind
    auto rewind_enabled = std::make_shared<OptionListComponent<std::string>>(mWindow, _("REWIND"));
    rewind_enabled->add(_("AUTO"), "auto", GamestationConf::getInstance()->get(systemData->getName() + ".rewind") != "0" && GamestationConf::getInstance()->get(systemData->getName() + ".rewind") != "1");
    rewind_enabled->add(_("ON"),   "1",    GamestationConf::getInstance()->get(systemData->getName() + ".rewind") == "1");
    rewind_enabled->add(_("OFF"),  "0",    GamestationConf::getInstance()->get(systemData->getName() + ".rewind") == "0");
    systemConfiguration->addWithLabel(_("REWIND"), rewind_enabled);
    // autosave
    auto autosave_enabled = std::make_shared<OptionListComponent<std::string>>(mWindow, _("AUTO SAVE/LOAD"));
    autosave_enabled->add(_("AUTO"), "auto", GamestationConf::getInstance()->get(systemData->getName() + ".autosave") != "0" && GamestationConf::getInstance()->get(systemData->getName() + ".autosave") != "1");
    autosave_enabled->add(_("ON"),   "1",    GamestationConf::getInstance()->get(systemData->getName() + ".autosave") == "1");
    autosave_enabled->add(_("OFF"),  "0",    GamestationConf::getInstance()->get(systemData->getName() + ".autosave") == "0");
    ...
    systemConfiguration->addSaveFunc(
                ...
                if(ratio_choice->changed()){
                    GamestationConf::getInstance()->set(systemData->getName() + ".ratio",
                                                     ratio_choice->getSelected());
                }
		if(videoResolutionMode_choice->changed()){
                    GamestationConf::getInstance()->set(systemData->getName() + ".videomode",
                                                     videoResolutionMode_choice->getSelected());
                }
                if(rewind_enabled->changed()) {
                    GamestationConf::getInstance()->set(systemData->getName() + ".rewind",
                                                     rewind_enabled->getSelected());
                }
                if(smoothing_enabled->changed()){
                    GamestationConf::getInstance()->set(systemData->getName() + ".smooth",
                                                     smoothing_enabled->getSelected());
                }
                if(emu_choice->changed()) {
                    GamestationConf::getInstance()->set(systemData->getName() + ".emulator", emu_choice->getSelected());
                }
                if(core_choice->changed()){
                    GamestationConf::getInstance()->set(systemData->getName() + ".core", core_choice->getSelected());
                }
                if(autosave_enabled->changed()) {
                    GamestationConf::getInstance()->set(systemData->getName() + ".autosave",
  ...
  GuiSettings *s = new GuiSettings(mWindow, _("CONTROLLERS SETTINGS").c_str());
    ...
    row.makeAcceptInputHandler([window, this, s, showControllerResult] {
        window->pushGui(new GuiLoading(window, [] {
	      bool success = GamestationSystem::getInstance()->scanNewBluetooth();
	      return (void *) success;
    ...
    row.makeAcceptInputHandler([window, this, s] {
        GamestationSystem::getInstance()->forgetBluetoothControllers();
        window->pushGui(new GuiMsgBox(window,
...
std::shared_ptr<OptionListComponent<std::string>> GuiMenu::createRatioOptionList(Window *window,
    ...
    std::string currentRatio = GamestationConf::getInstance()->get(configname + ".ratio");
...
std::shared_ptr<OptionListComponent<std::string>> GuiMenu::createVideoResolutionModeOptionList(Window *window,
    ...
    std::string currentVideoMode = GamestationConf::getInstance()->get(configname + ".videomode");
    if (currentVideoMode.empty()) {
        currentVideoMode = std::string("auto");
    }

    std::vector<std::string> videoResolutionModeMap = GamestationSystem::getInstance()->getVideoModes();
...
std::vector<std::string> GuiMenu::getDecorationsSets()
{
	...
	fs::path paths[pathCount] = {
		"/usr/share/gamestation/datainit/decorations",
