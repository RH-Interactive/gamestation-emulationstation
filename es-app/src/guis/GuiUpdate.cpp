...
{
    std::pair<std::string,int> updateStatus = GamestationSystem::getInstance()->updateSystem(&mBusyAnim);
...
void GuiUpdate::threadPing() 
{
  std::vector<std::string> msgtbl;
        if(GamestationSystem::getInstance()->ping()){
            if(GamestationSystem::getInstance()->canUpdate(msgtbl)){
