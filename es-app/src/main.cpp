...
int main(int argc, char* argv[])
{
	...
	GamestationConf* gamestationConf = GamestationConf::getInstance();

#if ENABLE_KODI == 1
	if(gamestationConf->get("kodi.enabled") == "1" && gamestationConf->get("kodi.atstartup") == "1"){
		GamestationSystem::getInstance()->launchKodi(&window);
	}
#endif

	GamestationSystem::getInstance()->getIpAdress();

	// UPDATE CHECK THREAD
	if(gamestationConf->get("updates.enabled") == "1"){
		...
		while(SDL_PollEvent(&event))
		{
			switch(event.type)
			{
				...
				case SDL_QUIT:
					running = false;
					break;
				case GamestationSystem::SDL_FAST_QUIT | GamestationSystem::SDL_RB_REBOOT:
					running = false;
					doReboot = true;
					Settings::getInstance()->setBool("IgnoreGamelist", true);
					break;
				case GamestationSystem::SDL_FAST_QUIT | GamestationSystem::SDL_RB_SHUTDOWN:
					running = false;
					doShutdown = true;
					Settings::getInstance()->setBool("IgnoreGamelist", true);
					break;
				case SDL_QUIT | GamestationSystem::SDL_RB_REBOOT:
					running = false;
					doReboot = true;
					break;
				case SDL_QUIT | GamestationSystem::SDL_RB_SHUTDOWN:
					running = false;
					doShutdown = true;
					break;
			}
