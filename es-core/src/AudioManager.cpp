...
void AudioManager::themeChanged(const std::shared_ptr<ThemeData> &theme) {
    if (GamestationConf::getInstance()->get("audio.bgmusic") == "1") {
...
void AudioManager::resumeMusic() {
    this->init();
    if (currentMusic != NULL && GamestationConf::getInstance()->get("audio.bgmusic") == "1") {
...
void AudioManager::musicEnd() {
    ...
    if (runningFromPlaylist && GamestationConf::getInstance()->get("audio.bgmusic") == "1") {
        playRandomMusic();
    }
}
...
void AudioManager::playCheckSound() {
	...
	if(boost::filesystem::exists(loadingMusic) == false){
	  loadingMusic = "/usr/share/gamestation/datainit/system/.emulationstation/themes/"+selectedTheme+"/fx/loading.ogg";
	}
