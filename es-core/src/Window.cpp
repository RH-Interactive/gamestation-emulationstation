...
void Window::input(InputConfig* config, Input input)
{
	...
	{
	  GamestationSystem::getInstance()->launchFileManager(this);
	}
#endif
	else
	{
...
#if ENABLE_KODI == 1
            if(config->isMappedTo("x", input) && input.value && !launchKodi && GamestationConf::getInstance()->get("kodi.enabled") == "1" && GamestationConf::getInstance()->get("kodi.xbutton") == "1"){
                launchKodi = true;
                Window * window = this;
